...
# $locale->text('Debit Note')
  ...
  if (@{ $form->{all_tipodoc} }) {
    $form->{selecttipodoc} = "\n";
    $form->{tipodoc} = "$form->{tipodoc}--$form->{tipodoc_id}";
    for (@{ $form->{all_tipodoc} }) {
	$form->{selecttipodoc} .= qq|$_->{description}--$_->{id}\n|
    }
  }
...
|;
  ...
  #$tipodoc = $form->hide_form(qw(tipodoc)); # armaghan: not needed here

  $tipodoc = qq|
              <tr>
	        <th align="right" nowrap>|.$locale->text('Tipo Documento').qq|</th>
		<td colspan=3><select name=tipodoc>|
		.$form->select_option($form->{selecttipodoc}, $form->{tipodoc}, 1).qq|
		</select>
		</td>
	      </tr>
|;
...
|;
  ...
  $form->hide_form(map { "select$_" } qw(formname currency department employee tipodoc projectnumber language paymentmethod));
...
sub post {
  ...
  # saqib 2-june-2008 custom document number taggging
  $oldinvtotal = $form->{oldinvtotal} * 1;
  if ($form->{ARAP} eq 'AR'){
     $form->{custom} = 'AR' if $oldinvtotal >= 0;
     $form->{custom} = 'CN' if $oldinvtotal < 0;
  } else {
     # $form->{ARAP} eq 'AP'
     $form->{custom} = 'AP' if $oldinvtotal >= 0;
     $form->{custom} = 'DN' if $oldinvtotal < 0;
  }
...
|;
  ...
  if (@{ $form->{all_warehouse} }) {
    ...
    $form->{selectwarehouse} = qq|$myconfig{warehouse}--$myconfig{warehouse_id}| if $myconfig{warehouse} and $myconfig{role} eq 'user';
    $warehouse = qq|
            <tr>
...
|;
  ...
  if (@{ $form->{all_tipodoc} }) {
    $form->{selecttipodoc} = "\n";
    $form->{tipodoc} = "$form->{tipodoc}--$form->{tipodoc_id}";# if $form->{tipodoc_id};

    for (@{ $form->{all_tipodoc} }) {
        #$form->{selecttipodoc} = qq|$myconfig{tipodoc}--$myconfig{tipodoc_id}|;# if $myconfig{tipodoc} and $myconfig{role} eq 'user';
	$form->{selecttipodoc} .= qq|$_->{description}--$_->{id}\n|
    }
  }
    ...
    <td>
	  ...
	  <td>
		...
		<td colspan=3><select name=$form->{ARAP}>$form->{"select$form->{ARAP}"}</select></td>
	      ...
	      $invnumber
	      <tr>
		<th align=right nowrap>|.$locale->text('Trans Date From').qq|</th>
		<td colspan=3><input name=transdatefrom size=11 class=date title="$myconfig{dateformat}"> <b>|.$locale->text('To').qq|</b> <input name=transdateto size=11 class=date title="$myconfig{dateformat}"></td>
	      ...
	      $selectfrom
	      <tr>
		<th align=right nowrap>|.$locale->text('Due Date From').qq|</th>
		<td colspan=3><input name=duedatefrom size=11 class=date title="$myconfig{dateformat}"> <b>|.$locale->text('To').qq|</b> <input name=duedateto size=11 class=date title="$myconfig{dateformat}"></td>
	      </tr>
	    </table>
	  </td>
...
sub transactions {
  ...
  if ($form->{duedatefrom}) {
    $callback .= "&duedatefrom=$form->{duedatefrom}";
    $href .= "&duedatefrom=$form->{duedatefrom}";
    $option .= "\n<br>" if ($option);
    $option .= $locale->text('From')."&nbsp;".$locale->date(\%myconfig, $form->{duedatefrom}, 1);
  }
  if ($form->{duedateto}) {
    $callback .= "&duedateto=$form->{duedateto}";
    $href .= "&duedateto=$form->{duedateto}";
    $option .= "\n<br>" if ($option);
    $option .= $locale->text('To')."&nbsp;".$locale->date(\%myconfig, $form->{duedateto}, 1);
  }

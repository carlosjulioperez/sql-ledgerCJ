...
sub comprasats_report {
   ...
   my $query = qq|
	...
	AND rt.tiporet_id <> '3'
	AND rt.tiporet_id <> '9'
	AND rt.tiporet_id <> '10'
	AND rt.tiporet_id <> '11'
	ORDER BY 1,2,3;
   |;
   ...
   print_heading('Ret IVA 10');
   print_heading('Ret IVA 20');
   print_heading('Ret IVA 30');
   print_heading('Ret IVA 50');
   ...
   my $subtotal_retiva10 = 0;
   my $subtotal_retiva20 = 0;
   my $subtotal_retiva30 = 0;
   my $subtotal_retiva50 = 0;
   ...
   while ($ref = $sth->fetchrow_hashref(NAME_lc)){
	# update totals etc
        ...
        $subtotal_retiva10 += $ref->{retiva10};
        $subtotal_retiva20 += $ref->{retiva20};
        $subtotal_retiva30 += $ref->{retiva30};
        $subtotal_retiva50 += $ref->{retiva50};
	...
	print_number($ref->{retiva10});
	print_number($ref->{retiva20});
	print_number($ref->{retiva30});
	print_number($ref->{retiva50});
   ...
   print_text('&nbsp;');
   print_text('&nbsp;');
   print_number($subtotal_retiva10);
   print_number($subtotal_retiva20);
   print_number($subtotal_retiva30);
   print_number($subtotal_retiva50);

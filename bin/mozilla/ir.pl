...
sub invoice_links {
  ...
  for (qw(partsgroup projectnumber department warehouse tipodoc employee language paymentmethod)) { $form->{"select$_"} = $form->escape($form->{"select$_"},1) }
...
#  $tipodoc = $form->hide_form(qw(tipodoc));
...
|;
  ...
  $form->hide_form(map { "select$_" } qw(formname currency partsgroup projectnumber department warehouse tipodoc employee language paymentmethod));
    ...
    <td>
	  ...
	  <td align=right>
		...
		<td><input name=invnumber size=20 value="|.$form->quote($form->{invnumber}).qq|"></td>
	      ...
	      <tr>
		<th align=right nowrap>|.$locale->text('Número de Autoriz').qq|</th>
		<td><input name=ordnumber size=11 value="|.$form->quote($form->{ordnumber}).qq|"></td>
	      </tr>
	      <tr>
...
sub post {
  ...
  # saqib 2-june-2008 custom document number tagging
  my $oldinvtotal = $form->{oldinvtotal} * 1;
  $form->{custom} = 'PI' if $oldinvtotal >= 0;
  $form->{custom} = 'PR' if $oldinvtotal < 0; 

  $form->isblank("transdate", $locale->text('Invoice Date missing!'));
  $form->isblank("vendor", $locale->text('Vendor missing!'));
  $form->isblank("department", $locale->text('Department missing!')) if $form->{selectdepartment};
  $form->isblank("tipodoc", $locale->text('TipoDoc missing!')) if $form->{selecttipodoc};

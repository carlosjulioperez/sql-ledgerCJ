...
.qq|\npurchase_order2--|.$locale->text('Purchase Order2')
.qq|\npurchase_order3--|.$locale->text('Purchase Order3')
.qq|\npurchase_order4--|.$locale->text('Purchase Order4')
  ...
  if ($form->{type} !~ /_quotation$/) {
    $ordnumber = qq|
		...
		.$form->hide_form(qw(quonumber)).qq|
	      ...
	      <tr>
		<th width=70% align=right nowrap>|.$locale->text('Quotation').qq|</th>
  ...
  <tr>
    <td>
	  ...
	  <td>
		...
		<tr><th colspan="2">----- Manufacturer freight -----</th></tr>
	 	<tr>
		   <th align=right>|.$locale->text('Internal Freight').qq|</td>
		   <td><input name=internal_freight size=10 value='$form->{internal_freight}'></td>
		</tr>

		<tr><th colspan="2">----- Shipping Cost -----</th></tr>
	 	<tr>
		   <th align=right>|.$locale->text('Shipping Freight').qq|</td>
		   <td><input name=shipping_freight size=10 value='$form->{shipping_freight}'></td>
		</tr>
	 	<tr>
		   <th align=right>|.$locale->text('Insurance').qq|</td>
		   <td><input name=shipping_insurance size=10 value='$form->{shipping_insurance}'></td>
		</tr>

		<tr><th colspan="2">----- Local Cost -----</th></tr>
	 	<tr>
		   <th align=right>|.$locale->text('Airport Expenses').qq|</td>
		   <td><input name=airport_expenses size=10 value='$form->{airport_expenses}'></td>
		</tr>
	 	<tr>
		   <th align=right>|.$locale->text('Local Freight').qq|</td>
		   <td><input name=local_freight size=10 value='$form->{local_freight}'></td>
		</tr>
		<tr>
		   <th align=right>|.$locale->text('Other Expenses').qq|</td>
		   <td><input name=other_expenses size=10 value='$form->{other_expenses}'></td>
	        </tr>
		<tr>
		   <th align=right>|.$locale->text('Total Expenses').qq|</td>
		   <td><input name=total_expenses size=10 value='$form->{total_expenses}'></td>
	        </tr>
		<tr>
		   <th align=right>|.$locale->text('Total Customs').qq|</td>
		   <td><input name=total_customs size=10 value='$form->{total_customs}'></td>
	        </tr>
		<tr>
		   <th align=right>|.$locale->text('G.W.').qq|</td>
		   <td><input name=grossweight size=10 value='$form->{grossweight}'></td>
	        </tr>
		<tr>
		   <th align=right>|.$locale->text('N.W.').qq|</td>
		   <td><input name=netweight size=10 value='$form->{netweight}'></td>
	        </tr>
...
|;
    ...
    $subtotal = qq|
	      <tr>
		<th align=right>|.$locale->text('Total N.W.').qq|</th>
		<td align=right>|.$form->format_amount(\%myconfig, $form->{total_netweight}, 2).qq|</td>
	      </tr>
	      <tr>
...
sub update {
  ...
  $form->{netweight} = 0;
  $form->{grossweight} = 0;
  $form->{total_customs} = 0;
  $form->{total_expenses} = $form->{airport_expenses}*1 + $form->{local_freight}*1 + $form->{other_expenses}*1;
  for (1 .. $form->{rowcount}) {
     $form->{"sellprice_$_"} = $form->parse_amount(\%myconfig, $form->{"sellprice_$_"});
     $form->{"sellprice2_$_"} = $form->parse_amount(\%myconfig, $form->{"sellprice2_$_"});

     $form->{netweight} += $form->parse_amount(\%myconfig, $form->{"netweight_$_"});
     $form->{total_customs} += ($form->{"customs_price_$_"} * $form->{"qty_$_"});
  }
  $form->{grossweight} = $form->{netweight};
  ...
  if (($form->{"partnumber_$i"} eq "") && ($form->{"description_$i"} eq "") && ($form->{"partsgroup_$i"} eq "")) {
	...
	for (keys %{ $form->{item_list}[0] }) { $form->{"${_}_$i"} = $form->{item_list}[0]{$_} }
	  ...
	  $decimalplaces1 = ($dec > $form->{precision}) ? $dec : $form->{precision};
 	  ...
 	  $form->{"sellprice2_$i"} = $form->{"sellprice_$i"};
	  ($dec) = ($form->{"sellprice2_$i"} =~ /\.(\d+)/);
	  $dec = length $dec;
	  $decimalplaces1 = ($dec > $form->{precision}) ? $dec : $form->{precision};

	  $form->{"sellprice2_$i"} /= $form->{exchangerate};
	}
    ...
    # Update packing material item.
  	for (1 .. $form->{rowcount}) {
	  $form->{"grossweight_$_"} = $form->{"grossweight_$i"} - $form->{netweight};
  	}
...
sub invoice {
  ...
  if ($form->{type} eq 'purchase_order') {
    for $i (1 .. $form->{rowcount}) { $form->{"sellprice_$i"} = $form->round_amount($form->{"newcost_$i"}, 2) }
  }

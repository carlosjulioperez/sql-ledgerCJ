...
#   $callback .= "&l_subtotal=$form->{l_subtotal}";
   ...
   while (my $ref = $sth->fetchrow_hashref(NAME_lc)){
	...
	$column_data{no}   		= qq|<td>$no <input type=checkbox value="Y" name="no_$no"><input type=hidden name="id_$no" value="$ref->{id}"></td>|;
   ...
   print('<hr size=3 noshade>');
   print qq|<input type=hidden name=rowcount value="$no">|;
   print qq|<input type=submit class=submit value="Delete" name="action" onclick="return confirm('Delete?');">|;
...
sub delete {
   ...
   $form->{title} = $locale->text('Retentions Deleted');
   &print_title;
   for $i (1 .. $form->{rowcount}){
      if ($form->{"no_$i"}){
         $db->query("DELETE FROM retenc WHERE id=?", $form->{"id_$i"});
         print qq|<h2>Deleted $form->{"id_$i"}</h2>|;
      }
   }
}

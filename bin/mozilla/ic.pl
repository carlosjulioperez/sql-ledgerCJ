...
sub link_part {
  ...
  foreach $ref (@{ $form->{customermatrix} }) {
    ...
    for (qw(validfrom validto pricebreak customerprice customerdiscount customercurr)) { $form->{"${_}_$i"} = $ref->{$_} }
...
|;
    ...
    $bin = qq|
		...
		<td><input name=partnumber2 size=10 value="|.$form->quote($form->{partnumber2}).qq|"></td>
	      </tr>
	      <tr>
		<th align="right" nowrap="true">|.$locale->text('Fodinfa %').qq|</th>
		<td><input name=fodinfa size=8 value=$form->{fodinfa}></td>
	      </tr>
	      <tr>
		<th align="right" nowrap="true">|.$locale->text('Advaloren %').qq|</th>
		<td><input name=advaloren size=8 value=$form->{advaloren}></td>
	      </tr>
|;
    
    $lotschecked = ($form->{uselots} eq 'Y' ? 'checked' : '');
  ...
  <tr>
	...
	<tr>
	  ...
	  <th align=right nowrap>|.$locale->text('Linea Item').qq|</th>
	...
	<tr>
	  ...
	  <th align=right nowrap>|.$locale->text('Use Lots').qq|</th>
	  <td><input type=checkbox name=uselots value='Y' $lotschecked></td>
	</tr>
      </table>
    </td>
  </tr>
...
sub generate_report {
  ...
  # saqib 2-dec-2007 added transdate
...
|;
  ...
  foreach $ref (@{ $form->{parts} }) {
    ...
    # saqib 2-dec-2007 transdate added
...
|;
	...
	<tr>
	  ...
	  <td><input name="customerdiscount_$i" size=6 value=|.$form->format_amount(\%myconfig, $form->{"customerdiscount_$i"}, $form->{decimalplacessell}).qq|></td>
	  $currency
...
sub check_customer {
  ...
  @flds = qw(customer validfrom validto pricebreak customerprice customerdiscount pricegroup customercurr);
  @a = (); 
  $count = 0;

  for (qw(customerprice customerdiscount pricebreak)) { $form->{"${_}_$form->{customer_rows}"} = $form->parse_amount(\%myconfig, $form->{"${_}_$form->{customer_rows}"}) }

  for $i (1 .. $form->{customer_rows} - 1) {

    for (qw(customerprice customerdiscount pricebreak)) { $form->{"${_}_$i"} = $form->parse_amount(\%myconfig, $form->{"${_}_$i"}) }

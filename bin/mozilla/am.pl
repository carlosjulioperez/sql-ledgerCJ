...
sub company_logo {
  ...
  my $pendingdelivered; 
  if ($myconfig{warehouse_id}){
     my $dbh = $form->dbconnect(\%myconfig);
     my $query = qq|SELECT COUNT(*) 
		FROM trf 
		WHERE to_warehouse_id = $myconfig{warehouse_id} 
		AND delivereddate IS NULL|;
     ($pendingdelivered) = $dbh->selectrow_array($query);
     $pendingdelivered *= 1;
     $dbh->disconnect;
  }
  my $pendingmsg;
  $pendingmsg = qq|There are $pendingdelivered pending transfers for warehouse $myconfig{warehouse}.| if $pendingdelivered > 0;

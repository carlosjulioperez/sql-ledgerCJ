...
sub add_user {
  ...
  # saqib 24-nov-2007 defaults from webmin. Remove/update if user changes his database password.
  $form->{Pg_dbname} = "${DB}";
  $form->{Pg_dbuser} = "${USER}";
  $form->{Pg_dbpasswd} = "${PASS}";
  $form->{Pg_dbhost} = "localhost";
...
<table width=100%>
  ...
  <tr>
    ...
    $checked = "checked"; # Check Pg database by default.
    print qq|
  ...
  <tr>
	...
	<tr>
	  <td>
		...
		<th align=right>|.$locale->text('Password').qq|</th>
		<td><input type=password name=dbpasswd size=15 value='$form->{dbpasswd}'></td>
	      </tr>
	      <tr>

	  ...
	  <table>
	    <tr>
		  ...
		  <tr>
		    ...
		    <td nowrap width=70><input name=c91 type=checkbox class=checkbox value=1 checked> 91+</td>
		  </tr>
		  <tr>
		    <td nowrap width=70><input name=c15 type=checkbox class=checkbox value=1 checked> 15</td>
...
sub generate_ar_aging {
  ...
  $employee_name = $form->escape($form->{employee_name},1);
...
sub aging {
  ...
  $column_header{c91} = qq|<th class=listheading width=10% nowrap>91+</th>|;
  ...
  @c = qw(c0 c15 c30 c45 c60 c90 c91);
  ...
  if ($form->{employee}) {
      ...
      $option .= $locale->text('Salesperson')." : $employee";
      $employee = $form->escape($form->{emplloyee},1);
      $form->{callback} .= "&employee=$employee";
  }
...
|;
	...
	$column_data{total} = qq|<th class=listsubtotal align=right>|.$form->format_amount(\%myconfig, $subtotal, $form->{precision}, "&nbsp;").qq|</th>|;
	$subtotal = 0;
...
sub print {
  ...
  if ($form->{media} !~ /(screen|email)/) {
    ...
    $form->{"$form->{vc}_id"} = "" if $form->{media} eq 'screen';
    $form->{"statement_1"} = 1;
...
sub print_form {
  ...
  while (@{ $form->{AG} }) {
	...
	$form->{total} = 0;
	foreach $item (qw(c0 c15 c30 c45 c60 c90 c91)) {
	  $form->{$item} = ();
	  $form->{"${item}total"} = 0;
	}
	...
	for ("c0", "c15", "c30", "c45", "c60", "c90", "c91", "") { $form->{"${_}total"} = $form->format_amount(\%myconfig, $form->{"${_}total"}, $form->{precision}) }

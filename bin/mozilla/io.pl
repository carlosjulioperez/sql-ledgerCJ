...
sub display_row {
  ...
  @column_index = qw(runningnumber partnumber warehouse lotnum itemdetail description lineitemdetail qty);
  ...
  if ($form->{type} eq "purchase_order") {
    unshift @column_index, 'checkbox2';
    unshift @column_index, 'checkbox';
    # push @column_index, "ship";
  ...
  if ($form->{type} eq "purchase_order") {
     push @column_index, qw(unit sellprice sellprice2 discount linetotal newcost taxable expiry cb cb2);
  } else {
     push @column_index, qw(unit sellprice discount linetotal taxable expiry);
  }

  my $colspan = $#column_index + 1;

  $form->{invsubtotal} = 0;
  $form->{invsubtotal_tax} = 0;
  $form->{invsubtotal_notax} = 0;

  for (split / /, $form->{taxaccounts}) { $form->{"${_}_base"} = 0 }
  
  $column_data{checkbox} = qq|<th class=listheading width=1%>P<br/><input name="allcb" type=checkbox class=checkbox value="Y" $form->{allcb} onChange="CheckAll1();"></th>|;
  $column_data{checkbox2} = qq|<th class=listheading width=1%>X<br/><input name="allcb2" type=checkbox class=checkbox value="Y" $form->{allcb2} onChange="CheckAll2();"></th>|;
  $column_data{runningnumber} = qq|<th class=listheading nowrap>|.$locale->text('Item').qq|</th>|;
  $column_data{warehouse} = qq|<th class=listheading nowrap>|.$locale->text('Warehouse').qq|</th>|;
  ...
  #$column_data{sellprice2} = qq|<th class=listheading nowrap>|.$locale->text('Price2').qq|</th>|;
  ...
  #$column_data{newcost} = qq|<th class=listheading nowrap>|.$locale->text('New Cost').qq|</th>|;
  $column_data{lotnum} = qq|<th class=listheading nowrap>|.$locale->text('Lot').qq|</th>|;
  $column_data{expiry} = qq|<th class=listheading nowrap>|.$locale->text('Expiry').qq|</th>|;
  ...
  $column_data{taxable} = qq|<th class=listheading nowrap>|.$locale->text('Tx').qq|</th>|;
...
function CheckAll1(v) {
  ...
  for (i = 0; i < el.length; i++) {
    if (el[i].type == 'checkbox' && re.test(el[i].name)) {
      el[i].checked = frm.allcb.checked
    }
  }
}

function CheckAll2(v) {
  ...
  for (i = 0; i < el.length; i++) {
    if (el[i].type == 'checkbox' && re.test(el[i].name)) {
      el[i].checked = frm.allcb2.checked
    }
  }
}
...
|;
  ...
  $sku = $locale->text('SKU');
  $packagenumber = $locale->text('Comments'); # Was Packaging
  ...
  for $i (1 .. $numrows) {
    ...
    if ($form->{type} eq "purchase_order") {
      if ($form->{"cb_$i"}){
        $form->{"sellprice_$i"} = $form->{"sellprice2_$i"} * 1.026;
      } else {
        $form->{"sellprice_$i"} = $form->{"sellprice2_$i"}; # if !$form->{"sellprice_$i"};
      }
    }
    ...
    if (($rows = $form->numtextrows($form->{"description_$i"}, 46, 6)) > 1) {
      ...
      $column_data{description} = qq|<td><textarea name="description_$i" rows=$rows cols=46 wrap=soft>$form->{"description_$i"}</textarea>|;
    } else {
      $form->{"description_$i"} = $form->quote($form->{"description_$i"});
      $column_data{description} = qq|<td><input name="description_$i" size=48 value="|.$form->quote($form->{"description_$i"}).qq|">|;
    }
...
|;
    ...
    # calculations
    $allocation_rate = 1;
    $allocation_rate = $form->{"weight_$i"} / $form->{netweight} if $form->{netweight};

    $form->{"internal_freight_$i"} = $form->round_amount($form->{internal_freight} * $allocation_rate, 2);
    $form->{"internal_freight_$i"} = 0 if $form->{"cb2_$i"};

    $form->{"fob_price_$i"} = $form->{"sellprice_$i"} + $form->{"internal_freight_$i"};
    $form->{"shipping_freight_$i"} = $form->round_amount($form->{shipping_freight} * $allocation_rate, 2);
    $form->{"shipping_insurance_$i"} = $form->round_amount($form->{shipping_insurance} * ($form->{"fob_price_$i"} + $form->{"shipping_freight_$i"}) / 100, 2);

    $form->{"shipping_freight_$i"} = 0 if $form->{"cb2_$i"};
    $form->{"shipping_insurance_$i"} = 0 if $form->{"cb2_$i"};

    $form->{"cif_price_$i"} = $form->{"fob_price_$i"} + $form->{"shipping_freight_$i"} + $form->{"shipping_insurance_$i"};

    if ($form->{"partnumber_$i"}){
       ($advaloren, $fodinfa) = $db->query('SELECT advaloren, fodinfa FROM parts WHERE partnumber = ?', $form->{"partnumber_$i"})->list;
    }
    $form->{"advaloren_$i"} = $form->round_amount($form->{"cif_price_$i"} * $advaloren / 100, 2);
    $form->{"fodinfa_$i"} = $form->round_amount($form->{"cif_price_$i"} * $fodinfa / 100, 2);

    $form->{"customs_price_$i"} = $form->{"cif_price_$i"} + $form->{"advaloren_$i"} + $form->{"fodinfa_$i"};
    $customs_extended = $form->{"customs_price_$i"} * $form->{"qty_$i"};
    $form->{"total_expenses_$i"} = $form->round_amount($form->{total_expenses} * $allocation_rate, 2);
    $form->{"warehouse_price_$i"} = $form->{"customs_price_$i"} + $form->{"total_expenses_$i"};

    my $checked = ($form->{"cb_$i"}) ? 'checked' : '';
    my $checked2 = ($form->{"cb2_$i"}) ? 'checked' : '';
    $column_data{checkbox} = qq|<td><input type="checkbox" name="cb_$i" value="1" $checked></td>|;
    $column_data{checkbox2} = qq|<td><input type="checkbox" name="cb2_$i" value="1" $checked2></td>|;
    $column_data{runningnumber} = qq|<td><input name="runningnumber_$i" size=3 value=$i></td>|;
    $column_data{warehouse} = qq|<td><select name="warehouse_$i">| . $form->select_option($form->{selectwarehouse}, $form->{"warehouse_$i"}, 1) . qq|</select></td>|;
    ...
    $column_data{description} .= qq|<table>|;
    $column_data{description} .= qq|<tr><td>Weight</td><td>$form->{"weight_$i"}</td></tr>| if $form->{type} eq 'purchase_order';
    $column_data{description} .= qq|<tr><td>PrecioVta</td><td><input name="sellprice_$i" size=11 value=|.$form->format_amount(\%myconfig, $form->{"sellprice_$i"}, $decimalplaces, $zero).qq|>|;
    if ($form->{type} eq 'purchase_order'){
       $column_data{description} .= qq|<tr><td>PrecioVta 2</td><td><input name="sellprice2_$i" size=11 value="|.$form->format_amount(\%myconfig, $form->{"sellprice2_$i"}, $decimalplaces, $zero).qq|" >|;
       $column_data{description} .= qq|<tr><td>Interal Freight</td><td><input name="internal_freight_$i" size=11 value="|.$form->format_amount(\%myconfig, $form->{"internal_freight_$i"}, $decimalplaces, $zero).qq|" >|;
       $column_data{description} .= qq|<tr><td><b>FOB Price</b></td><td><input name="fob_price_$i" size=11 value="|.$form->format_amount(\%myconfig, $form->{"fob_price_$i"}, $decimalplaces, $zero).qq|" >|;

       $column_data{description} .= qq|<tr><td>Shipping Freight</td><td><input name="shipping_freight_$i" size=11 value="|.$form->format_amount(\%myconfig, $form->{"shipping_freight_$i"}, $decimalplaces, $zero).qq|" >|;
       $column_data{description} .= qq|<tr><td>Shipping Insurance</td><td><input name="shipping_insurance_$i" size=11 value="|.$form->format_amount(\%myconfig, $form->{"shipping_insurance_$i"}, $decimalplaces, $zero).qq|" >|;
       $column_data{description} .= qq|<tr><td><b>CIF Price</b></td><td><input name="cif_price_$i" size=11 value="|.$form->format_amount(\%myconfig, $form->{"cif_price_$i"}, $decimalplaces, $zero).qq|" >|;

       $column_data{description} .= qq|<tr><td>Advaloren (duty)</td><td><input name="advaloren_$i" size=11 value="|.$form->format_amount(\%myconfig, $form->{"advaloren_$i"}, $decimalplaces, $zero).qq|" >|;
       $column_data{description} .= qq|<tr><td>Fodinfa (duty)</td><td><input name="fodinfa_$i" size=11 value="|.$form->format_amount(\%myconfig, $form->{"fodinfa_$i"}, $decimalplaces, $zero).qq|" >|;
       $column_data{description} .= qq|<tr><td><b>Customs Price</b></td><td><input name="customs_price_$i" size=11 value="|.$form->format_amount(\%myconfig, $form->{"customs_price_$i"}, $decimalplaces, $zero).qq|" >|;

       $column_data{description} .= qq|<tr><td>Other Expenses</td><td><input name="total_expenses_$i" size=11 value="|.$form->format_amount(\%myconfig, $form->{"total_expenses_$i"}, $decimalplaces, $zero).qq|" >|;
       $column_data{description} .= qq|<tr><td><b>Warehouse Price</b></td><td><input name="warehouse_price_$i" size=11 value="|.$form->format_amount(\%myconfig, $form->{"warehouse_price_$i"}, $decimalplaces, $zero).qq|" >|;
    }
    $column_data{discount} = qq|<td align=right><input name="discount_$i" size=5 value=|.$form->format_amount(\%myconfig, $form->{"discount_$i"}, 2).qq|></td>|;
    $column_data{description} .= qq|<tr><td>Extendido</td><td>|.$form->format_amount(\%myconfig, $linetotal, $form->{precision}, $zero).qq|\n|;
    $column_data{description} .= qq|</table></td>\n\n|;
    $column_data{lotnum} = qq|<td><input name="lotnum_$i" size=15 value="|.$form->quote($form->{"lotnum_$i"}).qq|" title='$form->{"lotqty_$i"}'>$form->{"lotnumlist_$i"}</td>|;
    $column_data{expiry} = qq|<td><input name="expiry_$i" size=11 value="|.$form->quote($form->{"expiry_$i"}).qq|" title="$myconfig{dateformat}"></td>|;
    $column_data{bin} = qq|<td>$form->{"bin_$i"}</td>|;
    $column_data{onhand} = qq|<td>$form->{"onhand_$i"}</td>|;
    if ($form->{"taxaccounts_$i"} and $form->{taxaccounts}){
       $column_data{taxable} = qq|<td>x</td>|;
    } else {
       $column_data{taxable} = qq|<td>&nbsp;</td>|;
    }
...
<input type=hidden name="oldlotnum_$i" value="|.$form->quote($form->{"lotnum_$i"}).qq|">
|;

    $form->hide_form(map { "${_}_$i" } qw(orderitems_id id weight listprice lastcost taxaccounts pricematrix sku onhand lotqty bin assembly inventory_accno_id income_accno_id expense_accno_id));
...
|;
    ...
    # armaghan remove following two lines to activate their function
    $delivery = '';
    $itemnotes = '';
    $serial = '';
    $orderxref = '';
...
|;
    ...
    if ($form->{"taxaccounts_$i"}){
       $form->{invsubtotal_tax} += $linetotal;
    } else {
       $form->{invsubtotal_notax} += $linetotal;
    }
  }
...
sub validate_lotnum {
   ...
   for my $i (1 .. $form->{rowcount} - 1){
      if ($form->{"lotnum_$i"}){
	 #if ($form->{"id_$i"} ne ""){
	     $form->{"id_$i"} *= 1;
	     my ($null, $warehouse_id) = split(/--/, $form->{"warehouse_$i"});
	     $warehouse_id *= 1;
	     if ($form->{"lotnum_$i"} eq '%'){
                my $query = qq|SELECT lotnum, expiry, (qty+allocated) * -1 AS lotqty 
			       FROM lots 
		...
		while (my $ref = $sth->fetchrow_hashref(NAME_lc)){
		    $form->{"lotnumlist_$i"} .= "<br>$ref->{lotnum},$ref->{lotqty},$ref->{expiry}\n";
		}
		$sth->finish;
		&get_lotnum($i);
	     } else {
                my $query = qq|SELECT lotnum, expiry, (qty+allocated) * -1 AS lotqty 
			       FROM lots
			       WHERE lotnum = '$form->{"lotnum_$i"}'
			       AND parts_id = $form->{"id_$i"}
			       AND warehouse_id = $warehouse_id|;
	        ($form->{"lotnum_$i"}, $form->{"expiry_$i"}, $form->{"lotqty_$i"}) =
...
sub select_lotnum {
  my $i = shift;
  @column_index = qw(ndx warehouse lotnum expiry onhand);
  ...
  $column_data{lotnum} = qq|<th class=listheading>|.$locale->text('Lot').qq|</th>|;
  $column_data{expiry} = qq|<th class=listheading>|.$locale->text('Expiry').qq|</th>|;
  $column_data{onhand} = qq|<th class=listheading>|.$locale->text('Qty').qq|</th>|;
  
  # list items with radio button on a form
  $form->header;

  $title = $locale->text('Select Warehouse');
  $option = '<b>' . $locale->text('Number') . ' ' . $form->{"partnumber_$i"} . '</b>';
...
|;
  ...
  my $dbh = $form->dbconnect(\%myconfig);
  my %defaults = $form->get_defaults($dbh, \@{['production_warehouse_id']});
  for (keys %defaults) { $form->{$_} = $defaults{$_} }

  my $query = qq|SELECT w.id AS warehouse_id, w.description AS warehouse, l.lotnum, 
			l.expiry, (l.qty+l.allocated) * -1 AS onhand
		  FROM lots l
		  JOIN warehouse w ON (w.id = l.warehouse_id)
		  WHERE parts_id = $form->{"id_$i"}
		  AND qty+allocated < 0
		  AND l.warehouse_id <> $form->{production_warehouse_id}
		  ORDER BY expiry DESC|;
  ...
  while ($ref = $sth->fetchrow_hashref(NAME_lc)) {
    $k++;
    $column_data{ndx} = qq|<td><input name="ndx" class=radio type=radio value=$k></td>|;
    for (qw(warehouse lotnum expiry)) { $column_data{$_} = qq|<td>$ref->{$_}&nbsp;</td>| }
...
|;
    for (qw(warehouse_id warehouse lotnum expiry onhand)) {
      print qq|<input type=hidden name="new_${_}_$k" value="|.$form->quote($ref->{$_}).qq|">\n|;
    }
  }
  print qq|
      </table>
...
<input name=detailndx type=hidden value=$i>

|;

  $form->{action} = "lot_selected";
  $form->hide_form;
  
  print qq|
<input type=hidden name=nextsub value=lot_selected>
...
sub lot_selected {
   $form->{"warehouse_$form->{detailndx}"} = qq|$form->{"new_warehouse_$form->{ndx}"}--$form->{"new_warehouse_id_$form->{ndx}"}|;
   # Following line is patch for trf.pl lookup
   $form->{"from_warehouse"} = qq|$form->{"new_warehouse_$form->{ndx}"}--$form->{"new_warehouse_id_$form->{ndx}"}|;
   $form->{"lotnum_$form->{detailndx}"} = $form->{"new_lotnum_$form->{ndx}"};
   &update;
}
...
sub get_lotnum {
   ...
   $warehouse_id *= 1;
   $form->{"id_$i"} *= 1;
   my $query = qq|SELECT lotnum, expiry, (qty-allocated) AS lotqty 
		  FROM lots
		  WHERE parts_id = $form->{"id_$i"}
		  AND warehouse_id = $warehouse_id
		  AND qty+allocated < 0
   ...
   $form->{"lotnum_$i"} = $ref->{lotnum};
   $form->{"expiry_$i"} = $ref->{expiry};
   $form->{"lotqty_$i"} = $ref->{lotqty} * -1;
}
...
sub item_selected {
  ...
  for $j (1 .. $form->{lastndx}) {
	...
	$form->{weight} += ($form->{"weight_$i"} * $form->{"qty_$i"});
      ...
      $linetotal = $form->{"sellprice_$i"} - $form->{"discount_$i"};
	...
	$form->{"lastcost_$i"} = $form->format_amount(\%myconfig, $form->{"lastcost_$i"}, $decimalplaces2);
      ...
      $form->{"discount_$i"} = $form->format_amount(\%myconfig, $form->{"discount_$i"}, 2);
...
sub check_form {
  ...
  my $i;
  my $j;
  my @flds = qw(id runningnumber partnumber description partsgroup qty ship unit sellprice sellprice2 discount oldqty oldship orderitems_id bin weight listprice lastcost taxaccounts pricematrix sku onhand assembly inventory_accno_id income_accno_id expense_accno_id itemnotes reqdate deliverydate serialnumber ordernumber customerponumber projectnumber package netweight grossweight lineitemdetail lotnum warehouse);
  ...
  if ($form->{type}) {
    ...
    #   $form->{"sellprice_$i"} = $form->{"sellprice2_$i"} * 1.026 if $form->{"cb_$i"};
    #}
    $count = 0;
    @a = ();
    if ($form->{rowcount}) {
...
sub invoicetotal {
  ...
  for $i (1 .. $form->{rowcount}) {
    ...
    $sellprice = $form->parse_amount(\%myconfig, $form->{"sellprice_$i"}) - $form->{"discount_$i"};
...
sub print_form {
  ...
  if ($form->{formname} =~ /(purchase_order|purchase_order2|purchase_order3)/) {
    $inv = "ord";
    $due = "req";
...
# $locale->text('Quotation Date missing!')
  ...
  foreach $i (1 .. $form->{rowcount}) {
    push @a, map { "${_}_$i" } qw(partnumber description projectnumber partsgroup serialnumber ordernumber customerponumber bin unit itemnotes package lotnum expiry);
  }

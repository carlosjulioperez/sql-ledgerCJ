...
sub create_links {
  ...
  if (@{ $form->{all_tipoid} }) {
    $form->{selecttipoid} = qq|\n|;
    for (@{ $form->{all_tipoid} }) { $form->{selecttipoid} .= qq|$_->{description}--$_->{id}\n| }
    $form->{selecttipoid} = $form->escape($form->{selecttipoid},1);
  }
...
|;
   ...
   $tipoidlabel = $locale->text('Tipo ID');

   $tipoid = qq|
	        <th align=right>$tipoidlabel</th>
		<td><select name=tipoid>|
		.$form->select_option($form->{selecttipoid}, $form->{tipoid}, 1)
		.qq|</select>
		</td>
|;
...
|.$form->hide_form(qw(gender occupation));
  ...
  for (qw(html_header html_footer latex_header latex_footer)){ $form->{$_} =~ s/&nbsp;/&amp;nbsp;/gi }
...
<table width=100%>
  ...
  <tr><td>
   <table>
      <tr valign=top>
	<th align=left nowrap>|.$locale->text('HTML Header').qq|</th>
	<td><textarea name=html_header rows=4 cols=80>$form->{html_header}</textarea></td>
      </tr>
      <tr valign=top>
	<th align=left nowrap>|.$locale->text('HTML Footer').qq|</th>
	<td><textarea name=html_footer rows=4 cols=80>$form->{html_footer}</textarea></td>
      </tr>
      <tr valign=top>
	<th align=left nowrap>|.$locale->text('Latex Header').qq|</th>
	<td><textarea name=latex_header rows=4 cols=80>$form->{latex_header}</textarea></td>
      </tr>
      <tr valign=top>
	<th align=left nowrap>|.$locale->text('Latex Footer').qq|</th>
	<td><textarea name=latex_footer rows=4 cols=80>$form->{latex_footer}</textarea></td>
      </tr>
    </table>
  </td></tr>
  <tr>
...
|;
  ...
  $form->hide_form(map { "select$_" } qw(currency arap discount payment business tipoid pricegroup language employee paymentmethod));

...
sub fe {
	...
	my $salida = `/usr/local/fe/oy_fecola.jar FA $form->{id}`;
	print qq|
		<body>
...
sub gr {
	$form->header;
	my $salida = `/usr/local/fe/oy_fecola.jar GR $form->{id}`;
	print qq|
		<body>
		<h2>COMPROBANTES ELECTRONICOS - COLA DE AUTORIZACIONES </h2>
		<h4>$salida</h4>
...
sub reprint {
  ...
  for (qw(department employee tipodoc language month partsgroup project years)) { delete $form->{"all_$_"} }

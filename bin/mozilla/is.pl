...
.qq|\nbin_list--|.$locale->text('Bin List');
  ...
  if ($form->{id}) {
    ...
    foreach $ref (@{ $form->{invoice_details} } ) {
      ...
      $form->{"warehouse_$i"} = qq|$ref->{warehouse}--$ref->{warehouse_id}| if $ref->{warehouse_id};
      ...
      for (qw(partnumber sku description unit lotnum expiry)) { $form->{"${_}_$i"} = $form->quote($form->{"${_}_$i"}) }
      $form->{rowcount} = $i;
      $i++;
    }
  }
...
|;
  ...
  my $invnumber_readonly = ' READONLY' if $myconfig{role} ne 'admin';
  print qq|
<table width=100%>
  <tr class=listtop>
  ...
  <tr>
	...
	<tr valign=top>
	  ...
	  <td align=right>
		...
		<td><input name=invnumber size=20 value="|.$form->quote($form->{invnumber}).qq|" $invnumber_readonly></td>
	      </tr>
	      <tr>
		...
		<td><input name=duedate size=11 class=date title="$myconfig{dateformat}" value=$form->{duedate}></td>
	      ...
	      $terms
	      <tr>
		<th align=right nowrap>|.$locale->text('Orden de Compra').qq|</th>
...
|;
  ...
  $subtotal = qq|
	      <tr>
		<th align=right>|.$locale->text('Taxable Subtotal').qq|</th>
		<td align=right>|.$form->format_amount(\%myconfig, $form->{invsubtotal_tax}, $form->{precision}, 0).qq|</td>
	      </tr>
	      <tr>
		<th align=right>|.$locale->text('Non-taxable Subtotal').qq|</th>
		<td align=right>|.$form->format_amount(\%myconfig, $form->{invsubtotal_notax}, $form->{precision}, 0).qq|</td>
	      </tr>
|;
  ...
  <tr>
	...
	<tr valign=bottom>
	  <td>
	    <table>
	      <tr>
		<th align=left>|.$locale->text('Guia de Remision').qq|</th>
...
|;
  ...
  if ($form->{readonly}) {
    ...
    %button = ('Update' => { ndx => 1, key => 'U', value => $locale->text('Update') },
	       ...
	       'Post as new' => { ndx => 8, key => 'N', value => $locale->text('Post as new') },
	       'Print and Post as new' => { ndx => 9, key => 'W', value => $locale->text('Print and Post as new') },
	       'Sales Order' => { ndx => 10, key => 'L', value => $locale->text('Sales Order') },
	       'Schedule' => { ndx => 11, key => 'H', value => $locale->text('Schedule') },
	       'Delete' => { ndx => 12, key => 'D', value => $locale->text('Delete') },
	       'Void' => { ndx => 13, key => 'D', value => $locale->text('Void') },
	      );
	...
	for ("Post", "Print and Post", "Delete", "Void") { delete $button{$_} }
      }
...
sub update {
  ...
  for my $j (1 .. $form->{rowcount} - 1){
    if (($form->{"partnumber_$j"} ne "") and ($form->{"id_$j"} ne "") and ($form->{"warehouse_$j"} eq "")){
      &select_lotnum($j);
      exit;
    }
  }
...
sub post {
  ...
  my $oldinvtotal = $form->{oldinvtotal} * 1;
  $form->{custom} = "001-002";
...
<h4>|.$locale->text('Are you sure you want to VOID Invoice Number').qq| $form->{invnumber}
</h4>

...
sub select_lineitem2 {
   my $query = qq|SELECT id, linumber, description FROM lineitem ORDER BY linumber|;
   my $dbh = $form->dbconnect(\%myconfig);
   $sth = $dbh->prepare($query);
   $sth->execute || $form->dberror($query);
   $form->{selectlineitem} = "<option>\n";
   while (my $ref = $sth->fetchrow_hashref(NAME_lc)){
      $form->{selectlineitem} .= qq|<option value="$ref->{linumber}--$ref->{id}">$ref->{linumber}--$ref->{description}\n|;
   }
   $dbh->disconnect;
}
...
sub inventory_search {
  print_header('Detalle de Ventas por Linea de Producto');
  begin_form;
  begin_table;

  select_warehouse;
#  select_lineitem;
#  print_select('Line Item', 'linenumber');
  print_select('Warehouse', 'warehouse');


#  $form->{partnumber} = ''; # default for search form.

  print_form_text('Linea Item', 'linenumber', 4 );
...
#   my ($lineitem, $lineitem_id) = split(/--/, $form->{lineitem});
#   my ($lineitem, $lineitem_linumber) = split(/--/, $form->{lineitem});
#   $lineitem_linumber *= 1;
#   $lineitem_id *= 1;
#   $where .= qq| AND p.tariff_hscode = $lineitem_id| if $form->{lineitem};

   my $linenumber = $form->like(lc $form->{linenumber}); 
   $where .= qq| AND LOWER(l.linumber) LIKE '$linenumber' | if $form->{linenumber};
   ...
   print qq|<tr><th>Linea</th><td>$lineitem</td></tr>| if $form->{lineitem};
   ...
   while ($ref = $sth->fetchrow_hashref(NAME_lc)){
	if ($linumber_group ne {linumber}){
	   if ($linumber_group ne 'none'){
		my $costquery = qq|SELECT avgcost
                                   FROM parts p
                                   join lineitem l ON (p.tariff_hscode = l.id)
                                   where l.linumber = '$linumber_group'|;

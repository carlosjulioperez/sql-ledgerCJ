...
sub form_header {
   ...
   $form->{selecttrftype} = qq|	<option value=transfer>transfer
				<option value=assembly>assembly
				<option value=components>components
   |;
   $form->{selecttrftype} =~ s/$form->{trftype}>/$form->{trftype} selected>/;
   ...
   &print_select('trftype', $locale->text('Type'));
   &print_text('ctrfnumber', $locale->text('Components Ref'), 10, "$form->{ctrfnumber}");
   ...
   for $i (1 .. $form->{rowcount}){
	...
	if (($form->{"partnumber_$i"} eq "") and ($i != $form->{rowcount})) {
	   ...
	   print $itemdetail;
	   print qq|<td><input name="lotnum_$j" type=text size=15 value='$form->{"lotnum_$i"}' title='$form->{"lotqty_$i"}'>$form->{"lotnumlist_$i"}</td>\n|;
	   print qq|<td><input name="description_$j" type=text size=48 value='$form->{"description_$i"}'></td>\n|;
	   print qq|<td><input name="qty_$j" type=text size=5 value='$form->{"qty_$i"}' title='$form->{"onhand_$i"}'></td>\n|;
	   print qq|<input type=hidden name="onhand_$j" value='$form->{"onhand_$i"}'>|;
	   ...
	   print qq|<td><input name="expiry_$i" type=text size=11 value='$form->{"expiry_$i"}' title='Expiry $myconfig{dateformat}'></td>\n|;
   ...
   &print_hidden('selectdepartment');
   &print_hidden('selectfrom_warehouse');
   &print_hidden('selectto_warehouse');
...
|;
   ...
   $form->hide_form(qw(closedto revtrans precision));
...
sub edit {
   ...
   my $query = qq|SELECT trf.id, trf.transdate, trf.trfnumber,
		...
		trf.from_warehouse_id, trf.to_warehouse_id, trf.trftype,
		trf.ctrfnumber
		FROM trf
   ...
   $query = qq|
	SELECT i.parts_id, p.partnumber, p.description,
		p.unit, i.qty, i.cost,
		i.itemnotes, i.serialnumber, lotnum, expiry
	FROM inventory i
...
sub lotcount {
  ...
  $warehouse_id *= 1;
  my $query = qq|SELECT COUNT(*) FROM lots
		  WHERE parts_id = $form->{"id_$i"}
		  AND qty+allocated < 0|;
  $query .= qq| AND warehouse_id = $warehouse_id| if $warehouse_id != 0;
...
sub update {
   ...
   if ($form->{trftype} ne 'assembly'){
     # Code to use validate_lotnum, select_lotnum procedures in io.pl
     for my $i (1 .. $form->{rowcount}){
        $form->{"warehouse_$i"} = $form->{from_warehouse};
        $form->{"id_$i"} = $form->{"parts_id_$i"};
     }
     for my $j (1 .. $form->{rowcount} - 1){
       if (($form->{"partnumber_$j"} ne "") and ($form->{"id_$j"} ne "") and ($form->{"lotnum_$j"} eq "")){
	  my $cnt = lotcount($j);
	  if ($cnt > 0){
            &select_lotnum($j);
            exit;
   ...
   if ($form->{trftype} eq 'assembly' and $form->{ctrfnumber} ne ''){
      my $dbh = $form->dbconnect(\%myconfig);
      $query = qq|SELECT SUM(i.qty * i.cost)
		FROM inventory i
		JOIN trf t ON (t.id = i.trans_id)
		WHERE (t.trfnumber = '$form->{ctrfnumber}')
		AND (i.linetype='1')
      |;
      my ($totalcost) = $dbh->selectrow_array($query);
      $form->{qty_1} *= 1;
      $form->{cost_1} = $totalcost / $form->{qty_1} if $form->{qty_1} != 0;
   }
...
sub yes {
  ...
  $query = qq|	SELECT i.parts_id, i.qty, t.trftype
		FROM inventory i
		JOIN trf t ON (t.id = i.trans_id)
  ...
  while (my $ref = $sth->fetchrow_hashref(NAME_lc)){
	if ($ref->{trftype} eq 'components'){
       		$form->update_balance($dbh, "parts", "onhand",
				qq|id = $ref->{parts_id}|,
				$ref->{qty});
	} elsif ($ref->{trftype} eq 'assembly') {
       		$form->update_balance($dbh, "parts", "onhand",
				qq|id = $ref->{parts_id}|,
				$ref->{qty} * -1);
	}
  ...
  $query = qq|DELETE FROM invoice WHERE trans_id = $form->{id}|;
  $dbh->do($query) || $form->dberror($query);
...
# armaghan Verbatim copy from IS.pm and then some changes for this form.
sub cogs {
  my ($dbh, $form, $id, $totalqty, $project_id, $c, $i) = @_;
...
sub save {
  ...
  if ($form->{id}){
	# Reverse quantities of 'components', 'assembly' transfer types in parts table.
	$query = qq|
		SELECT i.parts_id, i.qty, t.trftype
		FROM inventory i
		JOIN trf t ON (t.id = i.trans_id)
		WHERE trans_id = $form->{id}
		AND linetype = '1';
  ...
  $trf{ctrfnumber}   		= $form->{ctrfnumber};
  ...
  for $i (1 .. $j){
     ...
     my $id = 0;
     if ($form->{trftype} eq 'components' or $form->{trftype} eq 'assembly'){
        $query = qq|INSERT INTO invoice (description) VALUES ('$uid')|;
        $dbh->do($query) || $form->dberror($query);
        $query = qq|SELECT id FROM invoice WHERE description = '$uid'|;
        ($id) = $dbh->selectrow_array($query);
     }
     if ($form->{trftype} eq 'components'){
       $allocated = &cogs($dbh, $form, $form->{"parts_id_$i"}, $form->{"qty_$i"}, $project_id, "cost_$i", $i);
       # adjust parts onhand
       $form->update_balance($dbh, "parts", "onhand", 
				qq|id = $form->{"parts_id_$i"}|, 
				$form->{"qty_$i"} * -1);
        # Simulate sale of components removed from inventory for assembly.
        $query = qq|UPDATE invoice SET
		       	trans_id = $form->{trf_id},
			parts_id = $form->{"parts_id_$i"},
			description = '$form->{"description_$i"}',
			qty = $form->{"qty_$i"},
		 	allocated = 0,
			sellprice = $form->{"cost_$i"},
			fxsellprice = $form->{"cost_$i"},
			unit = '$form{"unit_$i"}',
			transdate = '$form->{transdate}',
			warehouse_id = $form->{from_warehouse_id},
			linetype = '4',
			lotnum = '$form->{"lotnum_$i"}'
		WHERE id = $id|;
       $dbh->do($query) || $form->dberror($query);

    } elsif ($form->{trftype} eq 'assembly'){
    	# Simulate purchase of newly built assemblies.
        $query = qq|UPDATE invoice SET
		       	trans_id = $form->{trf_id},
			parts_id = $form->{"parts_id_$i"},
			description = '$form->{"description_$i"}',
	...
	# update assembly
  	$form->update_balance($dbh, "parts", "onhand", 
			qq|id = $form->{"parts_id_$i"}|, 
			$form->{"qty_$i"});
        $query = qq|UPDATE parts SET lastcost = $form->{"cost_$i"}
			WHERE id= $form->{"parts_id_$i"}|;
        $dbh->do($query) || $form->dberror($query);

        $linetotal = $form->{"cost_$i"} * $form->{"qty_$i"};
	$query = qq|SELECT expense_accno_id, inventory_accno_id FROM parts WHERE id=$form->{"parts_id_$i"}|;
	my ($expense_accno_id, $inventory_accno_id) = $dbh->selectrow_array($query);

        # deduct expense
    	push @{ $form->{acc_trans}{lineitems} }, {
      		chart_id => $expense_accno_id,
      		amount => $linetotal,
      		project_id => $project_id,
      		source => 'ASM',
      		id => $ref->{id} };

    	# add inventory for goods made.
    	push @{ $form->{acc_trans}{lineitems} }, {
      		chart_id => $inventory_accno_id,
      		amount => $linetotal * -1,
      		project_id => $project_id,
      		source => 'ASM',
      		id => $ref->{id} };
     ...
     my %reporttypes = ('transfer' => 'TRF', 'assembly' => 'ASM', 'components' => 'COM');
     $inventory{trans_id}	= $form->{trf_id};
     $inventory{invoice_id}	= $id;
     ...
     $inventory{cogs}          = $form->{"cost_$i"} * $form->{"qty_$i"};
     ...
     $inventory{lotnum}		= $dbh->quote($form->{"lotnum_$i"});
     if ($form->{"expiry_$i"}){
        $form->{"expiry_$i"} = qq|'$form->{"expiry_$i"}'|;
     } else {
        $form->{"expiry_$i"} = 'NULL';
     }
     $inventory{expiry}		= $form->{"expiry_$i"};
     $inventory{reporttype}	= qq|'$reporttypes{$form->{trftype}}'|;
     ...
     if ($form->{"lotnum_$i"}){
       # Check if the to_warehouse/lotnum exits in lots table. If not add it.
       $query = qq|SELECT COUNT(*) FROM lots
		 WHERE lotnum = '$form->{"lotnum_$i"}'
		 AND parts_id = $form->{"parts_id_$i"}
		 AND warehouse_id = $form->{from_warehouse_id}
		|;
       my ($found) = $dbh->selectrow_array($query);

       if (!$found){
          $query = qq|INSERT INTO lots (lotnum, parts_id, 
				warehouse_id, expiry, qty, allocated)
                   VALUES ('$form->{"lotnum_$i"}', $form->{"parts_id_$i"}, 
                   $form->{from_warehouse_id}, $form->{"expiry_$i"}, 0, 0)|;
          $dbh->do($query) || $form->dberror($query);
       }
       ...
       $query = qq|SELECT COUNT(*) FROM lots
		 WHERE lotnum = '$form->{"lotnum_$i"}'
		 AND parts_id = $form->{"parts_id_$i"}
		 AND warehouse_id = $form->{to_warehouse_id}
		|;
       ($found) = $dbh->selectrow_array($query);

       if (!$found){
          $query = qq|INSERT INTO lots (lotnum, parts_id, warehouse_id, expiry, qty, allocated)
		    SELECT lotnum, parts_id, $form->{to_warehouse_id}, expiry, 0, 0
		    FROM lots
		    WHERE lotnum = '$form->{"lotnum_$i"}'
		    AND parts_id = $form->{"parts_id_$i"}
		    AND warehouse_id = $form->{from_warehouse_id}|;
	  $dbh->do($query) || $form->dberror($query);
       ...
       # IN in this warehouse
       $query = qq|SELECT SUM(0-qty) 
		     FROM inventory
		     WHERE parts_id = $form->{"parts_id_$i"}
		     AND lotnum = '$form->{"lotnum_$i"}'
		     AND warehouse_id = $form->{from_warehouse_id}
		     AND qty > 0|;
       my ($in) = $dbh->selectrow_array($query);
       # OUT from this warehouse
       $query = qq|SELECT SUM(0-qty) 
		     FROM inventory
		     WHERE parts_id = $form->{"parts_id_$i"}
		     AND lotnum = '$form->{"lotnum_$i"}'
		     AND warehouse_id = $form->{from_warehouse_id}
		     AND qty < 0|;
       my ($out) = $dbh->selectrow_array($query);

       $in *= 1; $out *= 1;
       $query = qq|UPDATE lots SET 
		     qty = $in, allocated = $out
		     WHERE parts_id = $form->{"parts_id_$i"}
		     AND lotnum = '$form->{"lotnum_$i"}'
		     AND warehouse_id = $form->{from_warehouse_id}|;
       $dbh->do($query) || $form->dberror($query);
  ...
  #use Data::Dumper; $form->info; print Dumper($form->{acc_trans}{lineitems}); $form->error;
  foreach $ref (sort { $b->{amount} <=> $a->{amount} } @ { $form->{acc_trans}{lineitems} }) {
    $amount = $ref->{amount};
    $query = qq|INSERT INTO acc_trans (trans_id, chart_id, amount,
		source, transdate)
		VALUES ($form->{trf_id}, $ref->{chart_id}, $ref->{amount},
	      '$ref->{source}', '$form->{transdate}')|;
    $dbh->do($query) || $form->dberror($query);
  }

...
sub create_links {
  ...
  if ($form->{id}) {
    $query = qq/SELECT ct.*,
		...
		tipoid.description || '--' || tipoid.id AS tipoid,
		...
		LEFT JOIN tipoid ON (ct.tipoid_id = tipoid.id)
  ...
  $query = qq|SELECT * FROM tipoid ORDER BY 1|;
  $sth = $dbh->prepare($query);
  $sth->execute || $form->dberror($query);
  while ($ref = $sth->fetchrow_hashref(NAME_lc)) {
    push @{ $form->{all_tipoid} }, $ref;
  }
  $sth->finish;
...
sub save {
  ...
  my %rec;
  for (qw(employee pricegroup business tipoid paymentmethod)) {
  ...
  $query = qq|UPDATE $form->{db} SET
	      ...
	      html_header = |.$dbh->quote($form->{html_header}).qq|,
	      html_footer = |.$dbh->quote($form->{html_footer}).qq|,
	      latex_header = |.$dbh->quote($form->{latex_header}).qq|,
	      latex_footer = |.$dbh->quote($form->{latex_footer}).qq|,
	      terms = $form->{terms},

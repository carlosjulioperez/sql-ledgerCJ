...
sub post_transaction {
  ...
  ($null, $form->{tipodoc_id}) = split(/--/, $form->{tipodoc});
  $form->{tipodoc_id} *= 1;
...
sub transactions {
  ...
  if (!$form->{summary} || $form->{l_memo}) {
    ...
    # get tipodoc
    $query = qq|SELECT * FROM tipodoc ORDER BY 1|;
    $sth = $dbh->prepare($query);
    $sth->execute || $form->dberror($query);
    while ($ref = $sth->fetchrow_hashref(NAME_lc)) {
      push @{ $form->{all_tipodoc} }, $ref;
    }
    $sth->finish;
...
#   	      LEFT JOIN tipodoc ON(a.tipodoc_id=tipodoc.id)
  ...
  for (qw(warehouse department employee tipodoc)) {
    if ($form->{$_}) {
  ...
  $where .= " AND a.duedate >= '$form->{duedatefrom}'" if $form->{duedatefrom};
  $where .= " AND a.duedate <= '$form->{duedateto}'" if $form->{duedateto};
...
####### Quitado porque daba error en pantalla ar/reports/transactions##################
####### creo que no es necesario para los reportes #####################
#                 e.tipodoc_id AS tipodoc_id, t.description AS tipodoc,
#               	 LEFT JOIN tipodoc t  ON(t.id = c.tipodoc_id)

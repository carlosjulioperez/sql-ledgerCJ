...
sub create_links {
  ...
  if ($self->{id}) {
    ...
    $query = qq|SELECT a.invnumber, a.transdate,
		...
		a.tipodoc_id, td.description AS tipodoc
		FROM $arap a
		JOIN $vc c ON (a.${vc}_id = c.id)
		LEFT JOIN employee e ON (e.id = a.employee_id)
		LEFT JOIN tipodoc td ON (td.id = a.tipodoc_id)
...
sub new {
  ...
  push @{ $self->{LONG_MONTH} }, ("Enero", "Febrero", "Marzo", "Abril", "Mayo ", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre");
  push @{ $self->{SHORT_MONTH} }, (qw(Ene Feb Mar Abr May Jun Jul Ago Sep Oct Nov Dic));

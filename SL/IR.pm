...
sub post_invoice {
  ...
  for (qw(department warehouse tipodoc)) {
  ...
  for my $i (1 .. $form->{rowcount}) {
	...
	$taxamount = $linetotal * $form->{"${_}_rate"} if $form->{"${_}_rate"} != 0; 
      ...
      $query = qq|INSERT INTO inventory (
			...
			$form->dbquote($form->{"expiry_$i"}, SQL_DATE) .qq|, 'CXP')|;
      $dbh->do($query) || $form->dberror($query);
      ...
      if ($form->{"lotnum_$i"}){ # lot processing
         ...
         my ($lotcount) = $dbh->selectrow_array($query);
         if (!($lotcount)){
            $query = qq|INSERT INTO lots (lotnum, parts_id, warehouse_id, expiry, qty, allocated)
                     VALUES ('$form->{"lotnum_$i"}', $form->{"id_$i"}, 
                       $form->{"warehouse_id_$i"}, |.$form->dbquote($form->{"expiry_$i"}, SQL_DATE).qq|, 0, 0)|;
             $dbh->do($query) || $form->dberror($query);
         }
  ...
  $query = qq|UPDATE ap set
	      ...
	      paymentmethod_id = $paymentmethod_id
              WHERE id = $form->{id}|;
  $dbh->do($query) || $form->dberror($query);
  $db->query('INSERT INTO cargo (id, trans_id, netweight, grossweight) VALUES (0, ?, ?, ?)', $form->{id}, $form->{netweight}, $form->{grossweight});
  ...
  foreach $item (keys %updparts) {
    ...
    $sth->execute($avgcost, $item) || $form->dberror;
    $sth->finish;
  ...
  if ($form->{id}) {
    ...
    $query = qq|SELECT a.invnumber, a.ordnumber, a.quonumber,
		...
		a.duedate, a.taxincluded, a.curr AS currency,
                a.tipodoc_id, tipodoc.description AS tipodoc,
		...
		LEFT JOIN tipodoc ON(a.tipodoc_id=tipodoc.id)

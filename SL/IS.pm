...
sub invoice_details {
  ...
  foreach $item (@sortlist) {
    ...
    if ($form->{"qty_$i"}) {
      ...
      $form->{"lotnum_$i"} = substr($form->{"lotnum_$i"}, 0, 10);

      for (qw(sku serialnumber ordernumber customerponumber bin description unit deliverydate sellprice listprice package netweight grossweight volume countryorigin hscode barcode itemnotes lotnum expiry)) { push(@{ $form->{$_} }, $form->{"${_}_$i"}) }
	...
	$form->{totalservices} += $linetotal;
      ...
      $subtotal += $linetotal;
      $form->{subtotal_tax} += $linetotal if $form->{"taxaccounts_$i"};
      $form->{subtotal_notax} += $linetotal if !$form->{"taxaccounts_$i"};

      $form->{"linetotal_$i"} = $form->format_amount($myconfig, $linetotal, $form->{precision}, "0");
      push(@{ $form->{linetotal} }, $form->{"linetotal_$i"});
      push(@{ $form->{taxable} }, 'x') if $form->{"taxaccounts_$i"};
	...
	if ($j < $k) {
	  ...
	  if ($sortlist[$j]->[1] ne $sameitem) {
	    ...
	    for (qw(taxrates runningnumber number sku serialnumber ordernumber customerponumber bin qty ship unit deliverydate projectnumber sellprice listprice netprice discount discountrate itemnotes package netweight grossweight volume countryorigin hscode barcode lotnum expiry)) { push(@{ $form->{$_} }, "") }
  ...
  $form->{invtotal} = $form->round_amount($form->{invtotal}, 6);
  ...
  $form->{cd_amount} = $form->round_amount($form->{cd_amount}, 6);
  ...
  for (qw(cd_subtotal cd_invtotal invtotal subtotal subtotal_tax subtotal_notax total totalparts totalservices)) { $form->{$_} = $form->format_amount($myconfig, $form->{$_}, $form->{precision}, "0") }
...
sub delete_invoice {
  ...
  if (!$form->{void}){
     # delete AR/AP record
     $query = qq|DELETE FROM ar WHERE id = $form->{id}|;
  } else {
     $query = qq|UPDATE ar SET amount = 0, netamount = 0, paid = 0, description = 'FACTURA ANULADA' WHERE id = $form->{id}|;
  }
  $dbh->do($query) || $form->dberror($query);
...
sub post_invoice {
  ...
  foreach $i (1 .. $form->{rowcount}) {
    ($null, $form->{"warehouse_id_$i"}) = split(/--/, $form->{"warehouse_$i"});
    $form->{"warehouse_id_$i"} *= 1;
	...
	($null, $project_id) = split /--/, $form->{"projectnumber_$i"};
      ...
      # deduct discount
      $form->{"sellprice_$i"} = $fxsellprice - $form->{"discount_$i"};
	...
	project_id => $project_id };
      ...
      if ($form->{"inventory_accno_id_$i"} || $form->{"assembly_$i"}) {
        ...
        # armaghan 12-Jan-2009 Now assemblies are treated as regular parts so the following code is commented out.
	# armaghan 23-Apr-2009 Reenabled again for old assemblies made and sold.
        if ($form->{"assembly_$i"}) {
      ...
      # armaghan 17-Mar-2008 per line tax amount for each tax
      for (@taxaccounts){
	my $taxamount = 0;
	$taxamount = $linetotal * $form->{"${_}_rate"} if $form->{"${_}_rate"} != 0; 
      ...
      # armaghan: 28-oct-2007 manage warehouse inventory from sale/purchase invoices
      $query = qq|INSERT INTO inventory (
			...
			lotnum, expiry, reporttype)
                VALUES ($form->{"warehouse_id_$i"}, $form->{"id_$i"}, $form->{id},
                        1, 0 - ($form->{"qty_$i"}), | .
			...
			$dbh->quote($form->{"lotnum_$i"}) . qq|, | .
			$form->dbquote($form->{"expiry_$i"}, SQL_DATE) .qq|, 'CXC')|;
      $dbh->do($query) || $form->dberror($query);

      if ($form->{"lotnum_$i"}){ # lot processing
         $query = qq|SELECT SUM(qty) 
		     FROM inventory
		     WHERE parts_id = $form->{"id_$i"}
		     AND lotnum = '$form->{"lotnum_$i"}'
		     AND warehouse_id = $form->{"warehouse_id_$i"}
		     AND qty > 0|;
         ...
         $in *= -1; $out *= 1;
         $query = qq|UPDATE lots 
		     SET qty = $in, allocated = $out
		     WHERE parts_id = $form->{"id_$i"}
		     AND lotnum = '$form->{"lotnum_$i"}'
		     AND warehouse_id = $form->{"warehouse_id_$i"}|;
         $dbh->do($query) || $form->dberror($query);
...
sub process_assembly {
  ...
  while (my $ref = $sth->fetchrow_hashref(NAME_lc)) {
    ...
    # TODO: Add individual items to inventory table too
...
sub retrieve_invoice {
  ...
  if ($form->{id}) {
    ...
    $query = qq|SELECT i.description, i.qty, i.fxsellprice, i.sellprice,
		...
		i.warehouse_id, w.description AS warehouse, i.lotnum, i.expiry,
...
sub retrieve_item {
  ...
  #   $where .= qq| AND p.id IN (SELECT parts_id
#				FROM inventory
...
sub price_matrix_query {
  ...
  my $query = qq|SELECT p.id AS parts_id, 0 AS customer_id, 0 AS pricegroup_id,
	      ...
	      '$form->{defaultcurrency}' AS curr, '' AS pricegroup, 0 AS discount
              FROM parts p
	      WHERE p.id = ?
	      ...
	      p.pricebreak, p.sellprice, p.validfrom, p.validto, 
	      p.curr, g.pricegroup, p.discount
	      ...
	      UNION
              ...
              SELECT p.parts_id, p.customer_id, p.pricegroup_id,
	      p.pricebreak, p.sellprice, p.validfrom, p.validto, 
	      p.curr, '' AS pricegroup, p.discount
	      FROM partscustomer p
...
sub price_matrix {
  ...
  while ($mref = $pmh->fetchrow_hashref(NAME_lc)) {
    ...
    $discount = $mref->{discount};

    $mref->{pricebreak} *= 1;

    if ($mref->{customer_id}) {
      $p{$mref->{pricebreak}} = $sellprice;
      $q{$mref->{pricebreak}} = $discount;
      $customerprice = 1;
    }

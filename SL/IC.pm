...
sub get_part {
  ...
  if ($form->{item} ne 'labor') {
    $query = qq|SELECT pc.pricebreak, pc.sellprice AS customerprice, pc.discount AS customerdiscount,
...
sub save {
  ...
  for (qw(rop weight listprice sellprice lastcost stock fodinfa advaloren)) { $form->{$_} = $form->parse_amount($myconfig, $form->{$_}) }
  
  $form->{assembly} = $form->{item} eq 'assembly';
  for (qw(alternate obsolete onhand assembly fodinfa advaloren)) { $form->{$_} *= 1 }
  ...
  $form->{uselots} = ($form->{uselots} eq 'Y') ? 'Y' : 'N';
  ...
  $query = qq|UPDATE parts SET
	      ...
	      partnumber2 = |.$dbh->quote($form->{partnumber2}).qq|,
	      ...
	      rop = $form->{rop},
	      fodinfa = $form->{fodinfa},
	      advaloren = $form->{advaloren},
  ...
  for $i (1 .. $form->{customer_rows}) {
    ...
    for (qw(pricebreak customerprice customerdiscount)) { $form->{"${_}_$i"} = $form->parse_amount($myconfig, $form->{"${_}_$i"})}
    ...
    if ($form->{"customerprice_$i"}) {
      ...
      $query = qq|INSERT INTO partscustomer (parts_id, customer_id,
                  pricegroup_id, pricebreak, sellprice, discount, curr,
		  validfrom, validto)
		  VALUES ($form->{id}, $customer_id,
		  $pricegroup_id, $form->{"pricebreak_$i"},
		  $form->{"customerprice_$i"}, $form->{"customerdiscount_$i"}, '$form->{"customercurr_$i"}',|
...
sub restock_assemblies {
  ...
  # armaghan 30-Mar-2008 Add row to build table 
  ($form->{employee}, $form->{employee_id}) = $form->get_employee($dbh);
...
# armaghan 10-Jan-2008 Verbatim copy of following procedure from IS.pm
sub cogs {
...
sub adjust_inventory {
  ...
  my $query = qq|SELECT p.id, p.inventory_accno_id, p.assembly, a.qty, p.description, p.unit
		 FROM parts p
		 JOIN assembly a ON (a.parts_id = p.id)
		 WHERE a.aid = $id|;
  my $sth = $dbh->prepare($query);
  ...
  $query = qq|SELECT description, unit FROM parts WHERE id = $id|;
  ($description, $unit) = $dbh->selectrow_array($query) || $form->dberror($query);
  ...
  while (my $ref = $sth->fetchrow_hashref(NAME_lc)) {
    ...
    # armaghan 30-Mar-2008 Add rows to inventory table for parts removed from stock for assembly
    ...
    # armaghan 10-Jan-2009 Simulate sale of components removed from inventory for assembly.
    $allocated = &cogs($dbh, $form, $ref->{id}, $ref->{qty}, $project_id);

    $query = qq|INSERT INTO invoice (
			trans_id, parts_id, description,
			qty, allocated, sellprice, fxsellprice, 
			unit, transdate, warehouse_id, linetype)
		VALUES ($form->{id}, $ref->{id}, '$ref->{description}',
			$qty * $ref->{qty}, $allocated, 0, 0,
			'$ref->{unit}', | . $form->dbquote($form->{transdate}, SQL_DATE).qq|, $form->{warehouse_id}, '4'
		)|;
    $dbh->do($query) || $form->dberror($query);
  ...
  # armaghan 30-Mar-2008 Add rows to inventory table for newly built assemblies.
  $query = qq|INSERT INTO inventory (
			warehouse_id, parts_id,
			trans_id, qty,
			shippingdate,
  ...
  # armaghan 10-Jan-2009 Simulate purchase of newly built assemblies.
  ## Get assembly lastcost as sellprice.
  my ($lastcost) = $dbh->selectrow_array("SELECT lastcost FROM parts WHERE id = $id");
  ## Add to invoice table.
  $query = qq|INSERT INTO invoice (
			trans_id, parts_id, description, assemblyitem,
			qty, allocated, sellprice, lastcost, fxsellprice,
			unit, transdate, warehouse_id, linetype)
		VALUES ($form->{id}, $id, '$description', TRUE,
			0 - $qty, 0, $lastcost, $lastcost, $lastcost,
			'$unit', | . $form->dbquote($form->{transdate}, SQL_DATE).qq|, $form->{warehouse_id}, '5'
 	   )|;
  $dbh->do($query) || $form->dberror($query);
...
sub all_parts {
  ...
  if ($form->{bought} || $form->{sold} || $form->{onorder} || $form->{ordered} || $form->{rfq} || $form->{quoted}) {
	...
	$invwhere .= " AND a.id = 0";
      ...
      # armaghan 6/11/2007 multiplied by -1 for correct sign
      ...
      if ($form->{bought}) {
	my $rflds = $flds;
        # armaghan 6/11/2007 disabled sign change

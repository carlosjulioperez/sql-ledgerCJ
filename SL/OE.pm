...
sub save {
  ...
  for my $i (1 .. $form->{rowcount}) {
    ...
    $form->{"sellprice2_$i"} = $form->parse_amount($myconfig, $form->{"sellprice2_$i"});
    $form->{"newcost_$i"} = $form->parse_amount($myconfig, $form->{"newcost_$i"});
	...
	($null, $project_id) = split /--/, $form->{"projectnumber_$i"};
      ...
      $form->{"newcost_$i"} *= 1;
      ($null, $warehouse_id) = split /--/, $form->{"warehouse_$i"};
      $warehouse_id *= 1;

      for (qw(internal_freight fob_price shipping_freight shipping_insurance cif_price advaloren fodinfa customs_price total_expenses warehouse_price)) { $form->{"${_}_$i"} *= 1 } 
      $query = qq|UPDATE orderitems SET
		  description = |.$dbh->quote($form->{"description_$i"}).qq|,
		  qty = $form->{"qty_$i"},
		  sellprice = $fxsellprice,
		  sellprice2 = $form->{"sellprice2_$i"},
		  newcost = $form->{"newcost_$i"},
		  discount = $form->{"discount_$i"},
		  internal_freight = $form->{"internal_freight_$i"},
		  fob_price = $form->{"fob_price_$i"},
		  shipping_freight = $form->{"shipping_freight_$i"},
		  shipping_insurance = $form->{"shipping_insurance_$i"},
		  cif_price = $form->{"cif_price_$i"},
		  advaloren = $form->{"advaloren_$i"},
		  fodinfa = $form->{"fodinfa_$i"},
		  customs_price = $form->{"customs_price_$i"},
		  total_expenses = $form->{"total_expenses_$i"},
		  warehouse_price = $form->{"warehouse_price_$i"},
		  ...
		  lotnum = |.$dbh->quote($form->{"lotnum_$i"}).qq|,
		  expiry = |.$form->dbquote($form->{"expiry_$i"}, SQL_DATE).qq|,
  ...
  $form->{terms} *= 1;
  for (qw(internal_freight shipping_freight shipping_insurance airport_expenses local_freight other_expenses total_expenses total_customs grossweight netweight)) { $form->{$_} *= 1 };
  # save OE record
  $query = qq|UPDATE oe set
	      ...
	      internal_freight = $form->{internal_freight},
	      shipping_freight = $form->{shipping_freight},
	      shipping_insurance = $form->{shipping_insurance},
	      airport_expenses = $form->{airport_expenses},
	      local_freight = $form->{local_freight},
	      other_expenses = $form->{other_expenses},
	      total_expenses = $form->{airport_expenses} + $form->{local_freight} + $form->{other_expenses},
	      grossweight = $form->{grossweight},
	      netweight = $form->{netweight},
	      total_customs = $form->{total_customs},
	      exchangerate = $form->{exchangerate}
              WHERE id = $form->{id}|;
  $dbh->do($query) || $form->dberror($query);
  $db->query('INSERT INTO cargo (id, trans_id, netweight, grossweight) VALUES (0, ?, ?, ?)', $form->{id}*1, $form->{netweight}*1, $form->{grossweight}*1);
...
sub retrieve {
  ...
  if ($form->{id}) {
    ...
    $query = qq|SELECT o.ordnumber, o.transdate, o.reqdate, o.terms,
		...
		o.aa_id, o.internal_freight, o.shipping_freight, o.shipping_insurance,
		o.airport_expenses, o.local_freight, o.other_expenses, o.total_expenses,
		o.grossweight, o.netweight, o.total_customs
		FROM oe o
    ...
    ($form->{netweight}, $form->{grossweight}) = $db->query('SELECT netweight, grossweight FROM cargo WHERE id=0 AND trans_id = ?', $form->{id})->list;
    ...
    $query = qq|SELECT o.id AS orderitems_id,
		...
		o.sellprice, o.sellprice2, o.newcost, o.parts_id AS id, o.unit, o.discount, p.bin,
                o.reqdate, o.project_id, o.ship, o.serialnumber,
		o.itemnotes, o.lineitemdetail, o.ordernumber,
		o.ponumber AS customerponumber, o.lotnum, o.expiry, o.warehouse_id, w.description AS warehouse,
		pr.projectnumber,
		...
		o.internal_freight, o.fob_price, o.shipping_freight, o.shipping_insurance, o.cif_price,
		o.advaloren, o.fodinfa, o.customs_price, o.total_expenses, o.warehouse_price, o.cb2
		FROM orderitems o
    ...
    while ($ref = $sth->fetchrow_hashref(NAME_lc)) {
      ...
      $ref->{cb} = '1' if $ref->{sellprice2} != $ref->{sellprice};

      $ref->{partsgroup} = $ref->{partsgrouptranslation} if $ref->{partsgrouptranslation};
      $ref->{warehouse} = "$ref->{warehouse}--$ref->{warehouse_id}";
...
sub price_matrix_query {
  ...
  if ($form->{customer_id}) {
    $query = qq|SELECT p.id AS parts_id, 0 AS customer_id, 0 AS pricegroup_id,
	     ...
	     '$form->{defaultcurrency}' AS curr, 0 AS discount, '' AS pricegroup
	     FROM parts p
	     WHERE p.id = ?
...
sub order_details {
  ...
  if ($form->{type} eq 'purchase_order'){
     ($form->{html_header}, $form->{html_footer}, $form->{latex_header}, $form->{latex_footer}) = 
	$db->query('SELECT html_header, html_footer, latex_header, latex_footer FROM vendor WHERE id = ?', 
	$form->{vendor_id})->list;
  }
  ...
  for $i (1 .. $form->{rowcount} - 1) {
    if ($form->{formname} =~ /purchase_order/){
	$form->{"description2_$i"} = $db->query("SELECT description FROM translation WHERE trans_id = ?", $form->{"id_$i"})->list;
        $form->{"description_$i"} .= qq|<br/>$form->{"description2_$i"}|;
    }

    $form->{"sellprice_$i"} = $form->{"fob_price_$i"} if $form->{formname} eq 'purchase_order2';
    $form->{"sellprice_$i"} = $form->{"cif_price_$i"} if $form->{formname} eq 'purchase_order3';
    $form->{"sellprice_$i"} = $form->{"warehouse_price_$i"} if $form->{formname} eq 'purchase_order4';
...
chop $form->{"description_$i"};
chop $form->{"description_$i"};
chop $form->{"description_$i"};
chop $form->{"description_$i"};
chop $form->{"description_$i"};

      for (qw(sku description unit bin serialnumber ordernumber customerponumber sellprice listprice package netweight grossweight volume countryorigin hscode drawing toolnumber barcode itemnotes lineitemdetail lotnum expiry)) { push(@{ $form->{$_} }, $form->{"${_}_$i"}) }

...
sub paymentaccounts {
  ...
  $sth->execute || $form->dberror($query);
  while ($ref = $sth->fetchrow_hashref(NAME_lc)) {
    push @{ $form->{all_cuenta} }, $ref;
  }
  $sth->finish;


  # tipos de retenciones
  $query = qq|SELECT * FROM tiporet ORDER BY 1|;
  $sth = $dbh->prepare($query);
  $sth->execute || $form->dberror($query);
  while ($ref = $sth->fetchrow_hashref(NAME_lc)) {
    push @{ $form->{all_tiporet} }, $ref;
  }
  $sth->finish;
...
sub post_payment {
  ...
  # armaghan: reference: ticket # 366
  if ($form->{if_ret} eq 'Y')
     for $i (1 .. $form->{rowcount2}){
        my ($null, $chart_id) = split(/--/, $form->{"cuenta$i"});
      
        my $query = qq|
	   INSERT INTO acc_trans(chart_id, transdate, amount)
	   VALUES ($chart_id, $form->{datepaid},  $form->{"valret$i"})
        |;
        # uncomment to do INSERTs
        # $dbh->do($query) || $form->dberror($query);

        # Remove following line after you are finished.
        $form->info("$query\n");
     }
  }
  ...
  for my $i (1 .. $form->{rowcount}) {
    ...
    for (qw(paid discount cuenta tiporet)) { $form->{"${_}_$i"} = $form->parse_amount($myconfig, $form->{"${_}_$i"}) }

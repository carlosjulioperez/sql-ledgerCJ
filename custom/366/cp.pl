...
sub edit {
  ...
  if (@{ $form->{all_tiporet} }) {
    $form->{selecttiporet} = "\n";
    $form->{tiporet} = "$form->{tiporet}--$form->{tiporet_id}";
    for (@{ $form->{all_tiporet} }) {
	$form->{selecttiporet} .= qq|$_->{description}--$_->{id}\n|
    }
  }
  ...
  if (@{ $form->{all_cuenta} }) {
    $form->{selectcuenta} = "\n";
    $form->{cuenta} = "$form->{cuenta}--$form->{cuenta_id}";
    for (@{ $form->{all_cuenta} }) {
	$form->{selectcuenta} .= qq|$_->{description}--$_->{id}\n|
    }
  }
...
sub payment {
  ...
  foreach $item (qw(department cuenta tiporet business paymentmethod)) {
    if (@{ $form->{"all_$item"} }) { 
      $form->{"select$item"} = "\n";
  ...
  $form->{rowcount2} = 1; # for cuenta tiporet
  ...
  for ("$form->{vc}", "$form->{ARAP}", "$form->{ARAP}_paid", "$form->{ARAP}_discount") { $form->{"select$_"} = $form->escape($form->{"select$_"},1) }
  for (qw(currency department business language account paymentmethod cuenta tiporet)) { $form->{"select$_"} = $form->escape($form->{"select$_"},1) }
...
|;
  ...
  $form->hide_form(map { "select$_" } qw(currency department business language account paymentmethod tiporet));
...
sub update {
  ...
  for $i (1 .. $form->{rowcount2}){
     if (($form->{"base0$i"} + $form->{"based0$i"} + $form->{"baseni$i"}) != 0){

	my ($null, $tiporet_id) = split(/--/, $form->{"tiporet$i"});
	$tiporet_id *= 1;
	my $dbh = $form->dbconnect(\%myconfig);
	$query = qq|SELECT porcret FROM tiporet WHERE id = $tiporet_id|;
 	($form->{"porcret$i"}) = $dbh->selectrow_array($query);
	$dbh->disconnect;

	$form->{"tiporet$j"} = $form->{"tiporet$i"}; 
	$form->{"cuenta$j"} = $form->{"cuenta$i"};

	$form->{"base0$j"} = $form->{"base0$i"};
	$form->{"based0$j"} =  $form->{"based0$i"};
	$form->{"baseni$j"} =  $form->{"baseni$i"};
	$form->{"porcret$j"} =  $form->{"porcret$i"};

	$form->{"valret$j"} = (($form->{"base0$j"} + $form->{"based0$j"} + $form->{"baseni$j"}) * $form->{"porcret$j"}) / 100;
	$form->{source} += $form->{"valret$j"};
	$j++;
     }
  }
...
sub payment_header {
  ...
  if ($form->{type} eq 'check') {
    $form->{title} = $locale->text('Payment & Retenciones');
  }
...
|;
  ...
  $ifret = ($form->{if_ret}) ? "checked" : "";
  $ifret = qq|
  	      <tr>
	        <td align=right>
		<input name=if_ret type=checkbox class=checkbox value=Y $ifret>
		<input type=hidden name="oldif_ret" value="$form->{if_ret}"></td>
		<th align=left>|.$locale->text('Retención').qq|</th>
	      </tr>
|;
...
|;
  ...
  $form->hide_form(qw(rowcount2 defaultcurrency closedto vc type ARAP arap title formname payment batch batchid batchnumber batchdescription transdate edit voucherid vouchernumber deselect employee precision));
  $form->hide_form("$form->{vc}_id");
  $form->hide_form(map { "old$_" } qw(currency datepaid duedatefrom duedateto department business paymentmethod));
  $form->hide_form(map { "old$_" } ("$form->{ARAP}", "$form->{vc}", "$form->{vc}number"));
  $form->hide_form(map { "select$_" } qw(currency department business paymentmethod cuenta tiporet));
    ...
    <td>
	  ...
	  <td align=right>
		...
		<input type=hidden name=oldamount value=|.$form->round_amount($form->{amount}, $form->{precision}).qq|>
    	    ...
    	    <table>
	      <tr>
		<th align=right nowrap>|.$locale->text('Retenc. Nbr:').qq|</th>
		<td colspan=3><input name=source value="|.$form->quote($form->{retnumber}).qq|" size=10></td>
	      </tr>

	      <tr>
		<th align=right nowrap>|.$locale->text('Autoriz. Nbr:').qq|</th>
		<td colspan=3><input name=source value="|.$form->quote($form->{ordnumber}).qq|" size=10></td>
	      </tr>
	    </table>
 	   </td>
  </tr>
  ...
  <table width=100%>
     <td align=center>
	  ...
	  <td>
	    <table>
	      <tr>
		<th align=left nowrap>|.$locale-> text('Tipo Retencion').qq|</th>
		<th align=center nowrap>|.$locale->text('Base 0      ').qq|</th>
		<th align=center nowrap>|.$locale->text('Base <> 0   ').qq|</th>
        	<th align=center nowrap>|.$locale->text('Base no Iva ').qq|</th>
        	<th align=center nowrap>|.$locale->text('% Retencion ').qq|</th>
        	<th align=center nowrap>|.$locale->text('Valor Reten.').qq|</th>
        	<th align=center nowrap>|.$locale->text('Cuenta.     ').qq|</th>
	      </tr>
|;

for $i (1 .. $form->{rowcount2}){
  $tiporet = qq|<td><select name=tiporet$i>|
		.$form->select_option($form->{selecttiporet}, $form->{"tiporet$i"}, 1).qq|
		</select>
	        </td>|;

  $cuenta = qq|<td><select name=cuenta$i>|
		.$form->select_option($form->{selectcuenta}, $form->{"cuenta$i"}, 1).qq|
		</select>
	        </td>|;

  print qq|<tr valign=left>
                $tiporet
                <td><input name=base0$i value="|.$form->quote($form->{"base0$i"}).qq|" size=12></td>
                <td><input name=based0$i value="|.$form->quote($form->{"based0$i"}).qq|" size=12></td>
                <td><input name=baseni$i value="|.$form->quote($form->{"baseni$i"}).qq|" size=12></td>
                <td><input name=porcret$i value="|.$form->quote($form->{"porcret$i"}).qq|" size=12></td>
                <td><input name=valret$i value="|.$form->quote($form->{"valret$i"}).qq|" size=12></td>
                $cuenta
	      </tr>|;
}
...
print qq|
       	    ...
       	    <table>
	      <tr>
		<th align=right nowrap>|.$locale->text('Total Retenido:').qq|</th>
		<td colspan=3><input name=source value="|.$form->quote($form->{source}).qq|" size=10></td>
	      </tr>
       	    </table>
